...
import { GroupRecoverCard } from "islands/webapp/GroupRecoverCard.tsx";
...
export default function DeletedGroupsList() {
  ...
  const groupsCards = filteredData.map((dict, _index) => (
    <GroupRecoverCard {...dict} />
  ));
  const desc =
    `Hello ${webAppData.value.user?.firstName}! Here you can manage the groups that have been deleted.`;
  return (
    <>
